...
export function flattenSingleAttribute(flattenedData, attributeName, value, type, margin = 0, id = "", directParentId = "", readOnlyChildren = type === "HTMLElement") {
  ...
  if (Array.isArray(value)) {
    value.forEach((val, index) => {
      ...
      flattenSingleAttribute(flattenedData, index, val, typeof val, margin + 10, `${elementId}.${index}`, elementId, readOnlyChildren)
    })
  } else if (value instanceof Object) {
    Object.entries(value).forEach(([objectKey, objectValue]) => {
      const elementId = id ? id : attributeName
      flattenSingleAttribute(flattenedData, objectKey, objectValue, typeof objectValue, margin + 10, `${elementId}.${objectKey}`, elementId, readOnlyChildren)
    })
  }
}
